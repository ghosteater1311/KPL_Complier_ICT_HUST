/* Scanner
 * @copyright (c) 2025, SoICT, Hanoi University of Science Technology
 * @author Huu-Duc Nguyen & Quang-Huy Duong
 * @version 2026
 */
...
int error(ErrorCode err, int lineNo, int colNo) {
  switch (err) {
...
Token* readIdentKeyword(void) {
  ...
  int count = 1;
  char keywordBuf[MAX_IDENT_LEN + 1];

  token->string[0] = (char)currentChar;
  keywordBuf[0] = toupper((char)currentChar);
  readChar();
  ...
  while ((currentChar != EOF) && 
    ...
    if (count <= MAX_IDENT_LEN) {
      ...
      keywordBuf[count] = toupper((char)currentChar);
      count++;
    }
    readChar();
  }
  ...
  keywordBuf[count] = '\0';
  token->tokenType = checkKeyword(keywordBuf);
...
Token* readNumber(void) {
  ...
  readChar();
  while (currentChar >= (int)'0' && currentChar <= (int)'9') {
    string[ index++] = (char)currentChar;
    readChar();
  }
  string[ index] = '\0';
...
int scan() {
  ...
  token = getToken();
  while (error_occurred == 0 && token->tokenType != TK_EOF) {
    printToken(token);
    free(token);
    token = getToken();
  }
